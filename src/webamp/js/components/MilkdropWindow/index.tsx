...
function Milkdrop({ analyser }: Props) {
  ...
  const currentPresetName = useTypedSelector(Selectors.getCurrentPresetName);
  const presetSelectionCount = useTypedSelector(Selectors.getPresetSelectionCount);
  ...
  const scheduleMilkdropMessage = useActionCreator(Actions.scheduleMilkdropMessage);

  const handleKeyDown = useKeyHandler();

  // Announce preset name when preset changes
  useEffect(() => {
    if (currentPresetName) {
      scheduleMilkdropMessage(currentPresetName);
    }
  }, [currentPresetName, scheduleMilkdropMessage]);
  ...
  }, [presetsAreCycling, currentPresetIndex, mediaIsPlaying, selectNextPreset, presetSelectionCount]);
