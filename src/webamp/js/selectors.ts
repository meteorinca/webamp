...
export function getPresetSelectionCount(state: AppState): number {
  return state.milkdrop.presetSelectionCount || 0;
}
...
export const getPresetNames = createSelector(
  getPresets,
  (presets): string[] => (presets ? presets.map((preset: any) => preset.name) : [])
);

export const getCurrentPresetName = createSelector(
  getPresets,
  getCurrentPresetIndex,
  (presets, index): string | null => {
    if (index == null || !presets || !presets[index]) return null;
